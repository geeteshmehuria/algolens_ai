...
.venv\Scripts\python.exe -m app.jobs.daily_import                        # run the daily problem-import job (external cron entrypoint)
```
...
- **`problem_import/`** (package) — the automated problem-import pipeline. `runner.run_import` orchestrates gather → rank → dedup → map-topic → persist → log, wrapping each source and each candidate in try/except so one failure can't abort a run (terminal status `success`/`partial`/`failed` written to `problem_import_runs`). `sources.py` adapters (`CuratedListSource` reads bundled `app/seed/curated_lists/*.json`; `AIGeneratedSource`; `ManualUploadSource`; `LeetCodeMetadataSource` is **off unless `PROBLEM_IMPORT_LEETCODE_ENABLED=true`** and fetches metadata only) all yield a normalized `ImportCandidate` and never raise out. `topic_map.TopicResolver` maps a coarse tag → a representative curriculum topic (lowest `learning_order` in the matching `TAG_TO_CATEGORY` category) + its first pattern. `dedup.find_duplicate` skips on `(source_name, leetcode_slug)`, any-source `leetcode_slug`, or same `title_slug` within the topic. `ranking` prioritizes easy-for-low-coverage via `TOPIC_COVERAGE_TARGETS`. `ai_content` generates original practice / learning packs (Pydantic-validated, cached in `ai_generated_content` kind `learning_pack`). **Legal rule baked in: only public metadata + AlgoLens-original AI content is stored — never third-party statements, editorials, hidden tests, or premium/company data.**
...
- **Imported-problem review gate**: `dsa_problems.import_status` is `published | review_required | archived`. Imports land as `review_required` (hidden); the public `GET /problems` only returns `published`, while the admin review queue (`GET /api/admin/problem-import/problems`) and publish/archive actions live under `routers/problem_import.py` (all `require_admin`). Existing/seeded rows are `source_type='seed'`/`published`. **The daily import runs as a standalone job (`app/jobs/daily_import.py`) invoked by an external scheduler, NOT in-process** — uvicorn `--reload` spawns multiple workers, so an in-process timer would fire once per worker and duplicate imports. Schema for it: migration `0007` (provenance columns + partial unique index `uq_problem_source_slug` + `problem_import_runs`).
- **Quiz answers never reach the client**: questions live in `topic_quiz_questions` separate from note content; grading is server-side at submit. MCQ/complexity are exact-match (no AI); all free-form answers (short_answer/dry_run) are graded in **one batched Gemini call** (`note_service._grade_free_form_batch`) — never one call per question — with exact-match fallback if AI is unavailable or omits an item.
...
- Topic-notes page (`routes/topics/[topicId]/notes/+page.svelte`) loads notes from a **single `$effect` keyed on `topicId`** (covers direct URL, refresh, back/forward, in-app switch) guarded by a monotonic `loadSeq` so a stale/duplicate response can't strand the spinner — don't reintroduce a parallel `onMount` load. Roles come from cached `loadContents()` (not a fresh `/auth/me`), and mutations (bookmark / section-read / mark-completed / quiz) **patch the cached sidebar entry locally** via `patchLocalTopic()` instead of refetching the whole `/me/topic-notes` list — only initial load and note generation refetch it.
