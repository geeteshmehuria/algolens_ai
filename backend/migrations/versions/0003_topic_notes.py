...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("topic_id", sa.Integer(), sa.ForeignKey("dsa_topics.id"), nullable=False),
        sa.Column("version", sa.Integer(), nullable=False, server_default="1"),
        sa.Column("status", sa.String(length=20), nullable=False, server_default="draft"),
        sa.Column("level", sa.String(length=30), nullable=False, server_default="beginner_to_intermediate"),
        ...
        sa.CheckConstraint("status IN ('draft', 'in_review', 'published', 'archived')", name="chk_topic_notes_status"),
        sa.CheckConstraint("source IN ('ai', 'ai_edited', 'human')", name="chk_topic_notes_source"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("note_id", sa.Integer(), sa.ForeignKey("topic_notes.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.UniqueConstraint("note_id", "position", name="uq_quiz_questions_note_position"),
        sa.CheckConstraint("kind IN ('mcq', 'short_answer', 'dry_run', 'complexity')", name="chk_quiz_questions_kind"),
    )
    ...
    op.create_table(
        ...
        sa.Column("note_id", sa.Integer(), sa.ForeignKey("topic_notes.id"), nullable=False),
        sa.Column("answers", sa.JSON(), nullable=False),
        sa.Column("score", sa.Integer(), nullable=False),
        sa.Column("total", sa.Integer(), nullable=False),
        sa.Column("created_on", sa.DateTime(), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ix_quiz_attempts_user", "user_quiz_attempts", ["user_id", "note_id"])
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('reading', 'completed', 'revised')", name="chk_user_note_state_status"),
    )
    op.create_index(op.f("ix_user_topic_note_state_user_id"), "user_topic_note_state", ["user_id"])
    ...
    op.create_index("ix_note_gen_log_topic", "ai_note_generation_log", ["topic_id", "created_on"])
    ...
    if binary_search_topic:
        ...
        note_content = {
            "sections": [
                {
                    ...
                    "examples": ["Find 9 in [-1,0,3,5,9,12]: check middle (3), too small, search right half..."],
                    "common_mistakes": ["Using it on unsorted data", "Infinite loop from `low = mid` instead of `low = mid + 1`"],
                    "interview_tips": ["Say the magic phrase: the array is sorted, so I can binary search in O(log n)."]
                },
                {
                    ...
                    "examples": [],
                    "common_mistakes": ["Not recognizing search-on-answer problems (Koko eating bananas style)"],
                    "interview_tips": ["Keyword clues: sorted, rotated, minimum capacity, smallest x such that..."]
                }
            ],
            ...
            "code_templates": [
                {
                    ...
                    "line_explanations": [{"lines": "4", "explanation": "Midpoint; Python ints do not overflow, in Java use low + (high-low)/2"}]
                }
            ],
            "complexity_notes": {
                "table": [{"operation": "Search", "time": "O(log n)", "space": "O(1)", "note": "Halves the space each step"}],
                "how_to_derive": "n -> n/2 -> n/4 ... reaches 1 after log2(n) steps.",
                "common_mistakes": ["Calling recursion depth O(1) space — recursive version is O(log n) stack"]
            },
            "practice_plan": {
                "one_day": ["Re-read revision notes", "Re-solve 1 classic + 1 boundary problem", "Dry-run the template on paper"],
                "seven_day": [
                    {
                        "day": 1,
            ...
            "confidence_checklist": [
                {"key": "explain_without_looking", "label": "I can explain binary search without looking at notes"},
                {"key": "boundary_variant", "label": "I can write the first-true boundary variant from scratch"},
                {"key": "complexity", "label": "I can derive O(log n) instead of just stating it"}
            ]
        }
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_user_topic_note_state_user_id"), table_name="user_topic_note_state")
