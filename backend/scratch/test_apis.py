...
def run_tests():
    ...
    status, res = make_request(
        "/api/auth/login", method="POST", data={"email": email, "password": password}
    )
    print(f"Status: {status}, Response: {res}")
    assert status == 200, f"Expected 200, got {status}"
    ...
    print(
        f"Status: {status}, Topics count: {len(res) if isinstance(res, list) else 'N/A'}"
    )
    assert status == 200, f"Expected 200, got {status}"
    ...
    print(
        f"Status: {status}, Patterns count: {len(res) if isinstance(res, list) else 'N/A'}"
    )
    assert status == 200, f"Expected 200, got {status}"
    ...
    status, res = make_request(
        "/api/problems", method="POST", data=problem_data, token=token
    )
    print(f"Status: {status}, Response: {res}")
    assert status == 201, f"Expected 201, got {status}"
    ...
    print(
        f"Status: {status}, Problems count: {len(res) if isinstance(res, list) else 'N/A'}"
    )
    assert status == 200, f"Expected 200, got {status}"
    ...
    status, res = make_request(
        f"/api/ai/generate-explanation/{problem_id}", method="POST", token=token
    )
    print(f"Status: {status}, Response: {res}")
    assert status == 200, f"Expected 200, got {status}"
    ...
    status, res = make_request(
        f"/api/ai/generate-animation/{problem_id}", method="POST", token=token
    )
    print(f"Status: {status}, Response: {res}")
    assert status == 200, f"Expected 200, got {status}"
    ...
    status, res = make_request(
        "/api/ai/hint",
        method="POST",
        data={"problem_id": problem_id, "hint_level": 1},
        token=token,
    )
    print(f"Status: {status}, Response: {res}")
    assert status == 200, f"Expected 200, got {status}"
    ...
    print(
        f"Status: {status}, Roadmaps count: {len(res) if isinstance(res, list) else 'N/A'}"
    )
    assert status == 200, f"Expected 200, got {status}"
    assert len(res) >= 1
